...
def setup_emulator(
    ...
    logging.info(
        f"Project: {project_id}, Topic: {topic_id}, Subscription: {subscription_id}"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Setup Pub/Sub emulator for local development"
    )
    parser.add_argument(
        "--project", default="local-project", help="Project ID for Pub/Sub emulator"
    )
    parser.add_argument(
        "--topic", default="demo-topic", help="Topic name for Pub/Sub emulator"
    )
    parser.add_argument(
        "--subscription",
        default="demo-topic-sub",
        help="Subscription name for Pub/Sub emulator",
    )
    parser.add_argument(
        "--emulator-host", default="localhost:8085", help="Pub/Sub emulator host:port"
    )
